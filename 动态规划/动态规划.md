...
* 动态规划+二分法。dp\[i][j]表示i层楼，j个鸡蛋的最小操作次数。O(knlogn)
            ...
            for j in range(2,k+1):
                ...
                while l <= r:
                    ...
                    # 蛋碎大于等于未碎的最小x，x-1即为未碎大于等于蛋碎的最大x
                        ...
                        l = mid
                        break
                dp[i][j] = min(dp[i][j],dp[l-1][j-1]+1,dp[i-l+1][j]+1)   
                '''
                枚举x   超时
            ...
            if (k,n) not in memo:
                ...
                while l <= r:	
                    ...
                    if t1 < t2:
                        ...
                        l = mid 
                        break
                res = 1 + min(recur(k-1,l-1),recur(k,n-l+1))
                memo[(k,n)] = res
            return memo[(k,n)]
        memo = {}
        return recur(k,n)
...
* 另一种状态描述，dp\[k][t]表示给定k枚鸡蛋和t次操作能检测的建筑高度，找到使dp\[k][t]>=n的最小t。O(kn)
